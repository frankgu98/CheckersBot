...
void getMove(int x, int y){ //takes a coordinate, checks the piece, determines all possible moves for that piece in the next two moves
    ...
    if (BOARD[x][y] == 2 || BOARD[x][y] == 3) { //checks if the piece belongs to robot
        ...
        if (BOARD[x][y] == 3){ //if the piece is a king
            ...
            for (int i = 0; i < 2; i++){
                ...
                x+2-4*i >= 0 && y-2 >= 0 && 
                x+2-4*i <= 7 && y-2 <= 7 &&
                BOARD[x+2-4*i][y-2] == 1){ //checks for existing backcaptures; same as above but y- instead of y+
                    int xNew = x+2-4*i, yNew = y-2; //new positions of piece
                    turnPoss[tCount].tiles[1].x = xNew; 
                    turnPoss[tCount].tiles[1].y = yNew; //saves this move
                    turnPoss[tCount].nCaps += 2; //capture increases to 1
                    
                    bool nextMove = false;
                    
                    for (int j = 0; j < 4; j++){ //if there was a capture, check for another possible capture
                        if (!(xNew+2*pow(-1,j) == x && yNew+2*pow(-1,j/2) == y)){ //prevents going back the same direction
                            if(xNew+pow(-1,j) >= 0 && yNew+pow(-1,j/2) >= 0 && //checks remaining 3 directions
                            xNew+pow(-1,j) <= 7 && yNew+pow(-1,j/2) <= 7 &&
                            BOARD[xNew+(int)(pow(-1,j))][yNew+(int)(pow(-1,j/2))] > 3 && 
                            xNew+2*pow(-1,j) >= 0 && yNew+2*pow(-1,j/2) >= 0 &&
                            xNew+2*pow(-1,j) <= 7 && yNew+2*pow(-1,j/2) <= 7 &&
                            BOARD[xNew+2*(int)(pow(-1,j))][yNew+2*(int)(pow(-1,j/2))] == 1){ //checks for legal captures, same as above
                                if (j){ //if this is the second run (j==1) and another capture is possible,
                                    turnPoss[tCount].tiles[1].x = xNew;
                                    turnPoss[tCount].tiles[1].y = yNew;
                                    turnPoss[tCount].nCaps += 2; //same as above
                                }
                                
                                turnPoss[tCount].tiles[2].x = xNew+2*pow(-1,j);
                                turnPoss[tCount].tiles[2].y = yNew+2*pow(-1,j/2);
                                turnPoss[tCount].nCaps++; //same as above
